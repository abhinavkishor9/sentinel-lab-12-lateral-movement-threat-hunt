# Investigation Notes — Sentinel Lab 12

## Investigation Goal

Investigate simulated Windows telemetry for evidence consistent with lateral movement between systems.

The investigation was performed using KQL `datatable()` because the available Sentinel workspace did not return the required `SecurityEvent` records.
...
The investigation identified multiple host-to-host access paths.

The `DESKTOP-LAB01 → SERVER-LAB02` sequence was the primary area of interest because authentication failures were followed by successful access, privileged activity, service creation, and command execution.

The `DESKTOP-LAB01 → SERVER-LAB03` sequence also warrants review because explicit credential use was followed by successful access and ADMIN$ activity.

The simulated data is sufficient to demonstrate the hunting methodology, but additional telemetry would be required for a real incident assessment.
